...
def maturity(date_maturity, as_of_date) -> float:
    '''
    ...
    delta = (date_maturity.year - as_of_date.year) * 12 + (date_maturity.month - as_of_date.month)

    # Add fractional part based on days
    day_fraction = (date_maturity.day - as_of_date.day) * 12/365
    residual_months = delta + day_fraction

    # If maturity date is before as_of_date, return 0 
    return max(residual_months, 0)


def nb_time_steps(residual_maturity: float, template_steps: pd.DataFrame) -> tuple[int, list]:
    '''
